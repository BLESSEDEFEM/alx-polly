 ...
 * 
 * This function performs comprehensive validation of poll data including:
 * - Question length and content validation
 * - Options count and content validation
 * - Prevention of empty or malformed data
 * 
 * @param {string} question - The poll question to validate
 * @param {string[]} options - Array of poll options to validate
 * @returns {string[]} Array of validation error messages (empty if valid)
 */
function validatePollInput(question: string, options: string[]): string[] {
  const errors: string[] = [];
 ...
 * 
 * This function removes potentially dangerous HTML/script content while
 * preserving safe text content for display.
 * 
 * @param {string} input - Raw string input to sanitize
 ...
 * @returns {Promise<{error: string | null, pollId?: string}>} Success/error response object
 */
export async function createPoll(formData: FormData) {
  ...
  return { error: null, pollId: data?.id };
}
